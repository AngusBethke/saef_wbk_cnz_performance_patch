...
_new_vv = _vv - 1;
if (_new_vv <= 0) exitWith {_zombie setDamage 1; [_zombie, "A_PlayerDeathAnim_11"] remoteExec ["switchMove", 0];};
...
{
if (!(_x == _unit) and (alive _x) and !(_x isKindOf "WBK_DOS_Squig_Normal") and !(_x isKindOf "WBK_Horses_ExportClass") and (isNil {_x getVariable "IMS_IsUnitInvicibleScripted"}) and !(_x isKindOf "WBK_C_ExportClass")) then {
...
_actFr = [{
	...
	if ((_mutant getVariable "canMakeAttack" == 0) and ((_myNearestEnemy distance _mutant) <= 4.2) and (alive _mutant) and !(animationState _mutant == "WBK_Middle_TryingToCatch") and !(animationState _mutant == "WBK_Middle_hit_b_1") and !(animationState _mutant == "WBK_Middle_hit_f_2_1") and !(animationState _mutant == "WBK_Middle_hit_f_1_1") and !(animationState _mutant == "WBK_Middle_Fall_Forward") and !(animationState _mutant == "WBK_Middle_Fall_Back")) then {
...
waitUntil {sleep 0.5; !(alive _mutant)};
